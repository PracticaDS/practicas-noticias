...
describe("routes", () => {
  ...
  beforeEach("Add mockdata to mockgoose", async() => {
    mockData.post1 = await new Post({ title: "Post #1", content: "Post #1 content", upvotes: 3 }).save()
    mockData.post2 = await new Post({ title: "Post #2", content: "Post #2 content" }).save()
  ...
  describe("GET /noticias", () => {
    ...
    it("Should return the two exiting posts", async() => {
    ...
    it("Comments should be sent as ids only", async() => {
      const response = await request(app)
            .get("/noticias")
            .expect(200)

      response.body[0].should.have.property("comments").with.lengthOf(2)
      response.body[0].comments[0].should.be.a("string")
      response.body[0].comments[1].should.be.a("string")
    })
